...
def handle_user_prompt_submit(input_data):
    ...
    if is_complex_task(user_prompt):
        log_handler_activity("compass_required", "Complex task detected - injecting COMPASS context")
...
def is_complex_task(prompt):
    ...
    code_patterns = [
        'class', 'function', 'method', 'algorithm', 'database', 'api', 
        'endpoint', 'integration', 'authentication', 'authorization',
        'framework', 'library', 'module', 'component', 'service'
    ]
    ...
    multi_step_count = sum(1 for word in multi_step_words if f' {word} ' in prompt_lower)
    ...
    if len(prompt_lower.split()) <= 3 and not any(trigger in prompt_lower for trigger in complexity_triggers):
        return False
...
def inject_compass_context():
    ...
    compass_context = """🧭 COMPASS METHODOLOGY REQUIRED
...
□ Step 5: Execute enhanced analysis with institutional context (compass-enhanced-analysis)
□ Step 6: Cross-reference findings with existing patterns (compass-cross-reference)

The compass-captain will coordinate these specialized agents to ensure:
...
def handle_pre_tool_use(input_data):
    ...
    if double_check:
        log_handler_activity("upstream_validation", f"Double-check requested for {tool_name}")
...
def predict_specialist_delegation(prompt):
    ...
    if any(keyword in prompt_lower for keyword in ['code', 'function', 'class', 'implement', 'refactor']):
        predicted_specialists.append('Code')
    
    # Task coordination
    if any(keyword in prompt_lower for keyword in ['coordinate', 'multi-step', 'complex', 'workflow']):
        predicted_specialists.append('Task')
    
    # Debugging specialists
    if any(keyword in prompt_lower for keyword in ['debug', 'error', 'issue', 'problem', 'troubleshoot']):
        predicted_specialists.append('Debugger')
    
    # Data analysis specialists
    if any(keyword in prompt_lower for keyword in ['data', 'analysis', 'query', 'sql', 'bigquery']):
...
def track_specialist_delegation_tokens(primary_agent, delegation_chain):
    ...
    for specialist_type in delegation_chain:
        ...
        specialist_context = f"Delegated task from {primary_agent} requiring {specialist_type} expertise"
...
def update_session_token_count(agent_type, token_count):
    ...
    try:
        ...
        with FileLock(f"{token_file}.lock"):
            ...
            session_tokens['by_agent'][agent_type] = session_tokens['by_agent'].get(agent_type, 0) + token_count
            session_tokens['last_update'] = datetime.now().isoformat()
            
            # Map agent to COMPASS phase
            phase = map_agent_to_phase(agent_type)
            if phase:
                session_tokens['by_phase'][phase] = session_tokens['by_phase'].get(phase, 0) + token_count
...
def handle_pre_tool_use_with_token_tracking(input_data):
    ...
    if tool_name == 'Task':
        ...
        if subagent_type.startswith('compass-'):
            ...
            estimated_tokens = estimate_agent_tokens(subagent_type, prompt, tool_input)
            
            # Track in session token counter
            update_session_token_count(subagent_type, estimated_tokens)
            
            # Check for specialist delegation
            if subagent_type == 'compass-coder':
                ...
                if predicted_specialists:
                    delegation_tokens = track_specialist_delegation_tokens(
                        subagent_type, predicted_specialists
                    )
                    update_session_token_count('delegation_chain', delegation_tokens)
            
            # Update user-visible progress
            update_compass_status_with_tokens(subagent_type, estimated_tokens)
            
            log_handler_activity("token_tracking", 
                f"{subagent_type}: {estimated_tokens} tokens estimated")
...
"""
        ...
        log_handler_activity("status_update_error", f"Failed to update status with tokens: {e}")
...
def generate_final_token_report():
    ...
    efficiency_percent = ((sequential_estimate - parallel_actual) / sequential_estimate * 100) if sequential_estimate > 0 else 0
...
def calculate_sequential_token_estimate(session_tokens):
    ...
    phase2_tokens = by_phase.get('phase2_pattern_application', 0) + \
                   by_phase.get('phase2_documentation_planning', 0) + \
                   by_phase.get('phase2_data_flow_analysis', 0)
    phase5_tokens = by_phase.get('phase5_cross_reference', 0) + \
                   by_phase.get('phase5_svg_analysis', 0)
    ...
    other_phases = sum(v for k, v in by_phase.items() 
...
def requires_compass_methodology(tool_name, tool_input):
    ...
    if tool_name == 'mcp__serena__read_file':
        ...
        if any(pattern in relative_path.lower() for pattern in ['src/', 'lib/', 'components/', 'services/', 'agents/']):
            return True
...
def compass_context_active():
        ...
        try:
            ...
            for line in recent_lines:
                try:
                    ...
                    if (log_entry.get('action') == 'compass_required' and 
                        is_recent_timestamp(log_entry.get('timestamp', ''))):
                        return True
    ...
    docs_dir = Path('docs')
    if docs_dir.exists():
        recent_files = [f for f in docs_dir.glob('*.md') 
...
def update_compass_phase(phase_name, status="in_progress"):
    ...
    if phase_name in phase_map:
        ...
        # Update timestamp
        content = re.sub(r"COMPLEX ANALYTICAL TASK DETECTED: .*", 
...
"""
        ...
        log_handler_activity("compass_complete", "Analysis completed - status cleaned up")
...
"""
        ...
        log_handler_activity("compass_complete", "Analysis completed with token tracking - status cleaned up")
...
"""
    ...
    log_handler_activity("status_file", "Created .compass-status with token tracking capabilities")
    ...
    if tool_name == 'Task':
        ...
        if subagent_type in agent_phase_map:
            ...
            if phase != 'coordination':  # Don't update for captain
                ...
                log_handler_activity("agent_active", f"Detected {subagent_type} activity - updating phase {phase}")
...
def generate_todo_update_context(subagent_type, phase):
    ...
    log_handler_activity("todo_update_generated", f"Generated todo update for {subagent_type} - {phase}")
...
def mark_compass_phase_complete(phase, subagent_type):
    ...
    log_handler_activity("phase_completed", f"Marked {phase} complete for {subagent_type}")
...
def trigger_upstream_validation(tool_name, tool_input):
    ...
    try:
        ...
        log_handler_activity("upstream_validation_triggered", f"Requesting validation for {tool_name}")
...
This is a double_check=true validation request requiring complete upstream verification."""
        ...
        log_handler_activity("upstream_validation_prepared", f"Task request prepared for {tool_name}")
